...
\begin{document}
	...
	\node (pas4) [trapeziSortida, below of=pas3, yshift=-2cm] {pas4\_infoPlan};
	\node (pas5) [trapeziSortida, below of=pas4] {pas5\_escullPlan};
	...
	\node (dummy4) [below of=pas4, xshift=0.9cm, yshift=1.2cm] {(/signup)};  
	...
	\node (dummy10) [below of=pas2b, xshift=1cm, yshift=1.2cm] {(/signup/password)};  
	\node (dummy11) [below of=pas2c, xshift=1cm, yshift=1.2cm] {(/login)};  


	%nodes de comentari nota al "peu" (node de refer�ncia)
	...
	\node (asterisc2Explicacio)[aspecteExplicacioAsterisc, right of = asterisc2Arribada, yshift=-1cm, xshift=1.8cm] {
		\begin{tabular}{l}
			Si pla\_suscripcio\_anual == 0:\\
				\hspace{2em}usuari no t\'{e} acc\'{e}s (i.e. no paga)\\
			Si pla\_suscripcio\_anual $>$ 0:\\
				\hspace{2em}usuari t\'{e} acc\'{e}s (i.e. paga o \'{e}s super usuari)			
		\end{tabular}
	};
